...
use kernel::{
    ...
    regmap,
    regulator::{
        driver::{Config, Desc, Operations, RegmapHelpers, Regulator, Status, Type},
        Mode,
    },
    sync::Arc,
};
...
regmap::regmap_register_fields!(REGS, {
    {
...
impl i2c::Driver for Driver {
    ...
    fn probe(client: &mut i2c::Client) -> Result<Self::Data> {
        ...
        let mut fields = regmap.alloc_fields(&REGS)?;
        ...
        let reg = Regulator::register(&dev, &NCV6336_DESC, config)?;
