...
struct Ncv6336(#[allow(dead_code)] Device<<Self as Driver>::Data>);
...
impl i2c::Driver for Ncv6336 {
    ...
    fn probe(
        ...
        let mut fields = regmap::Fields::new(&regmap, &FIELD_DESCS)?;

        dev_info!(
            client.as_ref(),
            "PID: {:#x}, RID: {:#x}, FID: {:#x}",
            pid::value::read(&mut fields)?,
            rid::value::read(&mut fields)?,
            fid::value::read(&mut fields)?,
        );
