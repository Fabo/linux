...
struct Ncv6336(#[allow(dead_code)] Registration);

impl i2c::Driver for Ncv6336 {
    type IdInfo = ();

    const I2C_TABLE: Option<i2c::IdTable<()>> = Some(&I2C_TABLE);
    const OF_TABLE: Option<of::IdTable<()>> = Some(&OF_TABLE);

    fn probe(client: &mut i2c::Client) -> Result<Pin<KBox<Self>>> {
