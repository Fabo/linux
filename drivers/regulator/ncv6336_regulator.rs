...
kernel::of_device_table!(
    ...
    OF_TABLE,
    (),
    [(of::DeviceId::with_compatible(c_str!("onnn,ncv6336")), ()),]
);
...
impl i2c::Driver for Ncv6336 {
    type Data = KBox<Registration>;

    const I2C_DEVICE_ID_TABLE: Option<i2c::IdTable<()>> = Some(&I2C_TABLE);
    const OF_DEVICE_ID_TABLE: Option<of::IdTable<()>> = Some(&OF_TABLE);
    ...
    fn probe(client: &mut i2c::Client) -> Result<Self::Data> {
        ...
        let registration = KBox::new(
            Registration::register(&client, &NCV6336_DESC, config)?,
            GFP_KERNEL,
        )?;
