...
type DeviceData = Mutex<TrulyR63350>;
...
impl mipi_dsi::Driver for TrulyR63350 {
    ...
    fn probe(mut dsi: mipi_dsi::Device, id_info: Option<&Self::IdInfo>) -> Result<Self::Data> {
        ...
        let regulators = [
            ...
            Regulator::get(dsi.as_ref(), c_str!("avee"))?,
            Regulator::get(dsi.as_ref(), c_str!("iovcc"))?,
        ];
...
#[vtable]
impl panel::Operations for TrulyR63350 {
    type Data = Arc<DeviceData>;

    fn prepare(data: <Self::Data as ForeignOwnable>::Borrowed<'_>) -> Result {
        let mut data = data.lock();
        let this: &mut Self = data.deref_mut();

        let reset_gpio = &mut this.reset_gpio;
        ...
        let mut regulators = ScopeGuard::new_with_data(
            KVec::with_capacity(this.regulators.len(), GFP_KERNEL)?,
            |v| {
        ...
        for regulator in &mut this.regulators {
            regulator.enable()?;
        ...
        *dsi.mode_flags_mut() |= mipi_dsi::mode::LPM;
        ...
        for cmd in this.pdata.panel_on_cmds {
    ...
    fn unprepare(data: <Self::Data as ForeignOwnable>::Borrowed<'_>) -> Result {
        ...
        *dsi.mode_flags_mut() &= !mipi_dsi::mode::LPM;
        ...
        for cmd in this.pdata.panel_off_cmds {
        ...
        if let Some(ref mut reset) = &mut this.reset_gpio {
            reset.set_value(1);
        }
...
kernel::of_device_table!(
    OF_ID_TABLE,
    MODULE_OF_ID_TABLE,
    <TrulyR63350 as mipi_dsi::Driver>::IdInfo,
    [
        (of::DeviceId::new(c_str!("auo,r63350-fhd")), &AUO_PDATA),
        (of::DeviceId::new(c_str!("truly,r63350-fhd")), &TRULY_PDATA),
    ]
);
