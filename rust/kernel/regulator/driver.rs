...
//!
//! ```ignore
//! use kernel::regulator::driver::{Config, Desc, Driver, Device, Type};
...
//!
//! static DESC: Desc =
//!     Desc::new::<MyDeviceDriver>(c_str!("my-regulator-driver"), Type::Voltage)
...
//!         let reg = Device::register(pdev, &DESC, Config::<Self::Data>::new(pdev))?;
//!         Ok(())
//!     }
//! }
//!
...
impl<T: ForeignOwnable + Send + Sync> Device<T> {
    /// SAFETY: `rdev` must be non-null.
    unsafe fn from_raw(rdev: *mut bindings::regulator_dev) -> ManuallyDrop<Self> {
        ManuallyDrop::new(Self {
            // SAFETY: The caller of `Self::from_raw must garantee that `rdev` is non-null.
...
impl<T: Driver> Adapter<T> {
    ...
    const VTABLE: bindings::regulator_ops = bindings::regulator_ops {
        ...
        ..unsafe { MaybeUninit::<bindings::regulator_ops>::zeroed().assume_init() }
    };
