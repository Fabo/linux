...
impl Regulator {
    ...
    /// created
    pub fn get(dev: &dyn RawDevice, id: &'static CStr) -> Result<Self> {
    ...
    pub fn get_optional(dev: &dyn RawDevice, id: &'static CStr) -> Result<Self> {
        // SAFETY: `dev.raw_device() is valid and non-null by the type invariant and
        // id has a static lifetime so it lives indefinitely
    ...
    pub fn get_exclusive(dev: &dyn RawDevice, id: &'static CStr) -> Result<Self> {
        // SAFETY: `dev.raw_device() is valid and non-null by the type invariant and
        // id has a static lifetime so it lives indefinitely
        let reg = from_err_ptr(unsafe {
