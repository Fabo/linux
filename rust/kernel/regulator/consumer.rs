...
///
/// # Invariants
///   - [`self.0.as_ptr()`] is valid and non-null
pub struct Regulator(NonNull<bindings::regulator>);
...
impl Regulator {
    ...
    pub fn get(dev: &Device, id: &'static CStr) -> Result<Self> {
        ...
        let reg = from_err_ptr(unsafe { bindings::regulator_get(dev.as_raw(), id.as_char_ptr()) })?;
        Ok(Self(NonNull::new(reg).ok_or(ENODEV)?))
    }
    ...
    pub fn get_optional(dev: &Device, id: &'static CStr) -> Result<Self> {
        ...
        let reg = from_err_ptr(unsafe {
            bindings::regulator_get_optional(dev.as_raw(), id.as_char_ptr())
        })?;
        Ok(Self(NonNull::new(reg).ok_or(ENODEV)?))
    }
    ...
    pub fn get_exclusive(dev: &Device, id: &'static CStr) -> Result<Self> {
        ...
        // id has a static lifetime so it lives indefinitely
        let reg = from_err_ptr(unsafe {
            bindings::regulator_get_exclusive(dev.as_raw(), id.as_char_ptr())
        })?;
        Ok(Self(NonNull::new(reg).ok_or(ENODEV)?))
    }
    ...
    pub fn is_enabled(&mut self) -> Result<bool> {
        ...
        let val = unsafe { bindings::regulator_enable(self.0.as_ptr()) };
        if val < 0 {
    ...
    pub fn enable(&mut self) -> Result {
        // SAFETY: The pointer is valid and non-null by the type invariant
        to_result(unsafe { bindings::regulator_enable(self.0.as_ptr()) })
    }

    /// Disable the regulator
    pub fn disable(&mut self) -> Result {
        // SAFETY: The pointer is valid and non-null by the type invariant
        to_result(unsafe { bindings::regulator_disable(self.0.as_ptr()) })
    }
    ...
    pub fn disable_deferred(&mut self, duration: Duration) -> Result {
        ...
        // SAFETY: The pointer is valid and non-null by the type invariant
        to_result(unsafe { bindings::regulator_disable_deferred(self.0.as_ptr(), ms) })
    }
    ...
    pub fn force_disable(&mut self) -> Result {
        // SAFETY: The pointer is valid and non-null by the type invariant
        to_result(unsafe { bindings::regulator_force_disable(self.0.as_ptr()) })
    }
    ...
    pub fn is_supported_voltage(&self, min_uv: c_int, max_uv: c_int) -> Result<bool> {
        // SAFETY: The pointer is valid and non-null by the type invariant
        let ret =
            unsafe { bindings::regulator_is_supported_voltage(self.0.as_ptr(), min_uv, max_uv) };
        if ret < 0 {
    ...
    pub fn count_voltages(&self) -> Result<usize> {
        // SAFETY: The pointer is valid and non-null by the type invariant
        let ret = unsafe { bindings::regulator_count_voltages(self.0.as_ptr()) };
        if ret < 0 {
            return Err(Error::from_errno(ret));
        }
    ...
    pub fn list_voltage(&self, selector: c_uint) -> Result<Option<c_int>> {
        // SAFETY: The pointer is valid and non-null by the type invariant
        let ret = unsafe { bindings::regulator_list_voltage(self.0.as_ptr(), selector) };
        if ret < 0 {
            return Err(Error::from_errno(ret));
        }
    ...
    pub fn get_linear_step(&self) -> Option<c_uint> {
        // SAFETY: The pointer is valid and non-null by the type invariant
        let ret = unsafe { bindings::regulator_get_linear_step(self.0.as_ptr()) };
        if ret == 0 {
            None
        } else {
            Some(ret)
    ...
    pub fn get_voltage(&self) -> Result<c_int> {
        // SAFETY: The pointer is valid and non-null by the type invariant
        let ret = unsafe { bindings::regulator_get_voltage(self.0.as_ptr()) };
        if ret < 0 {
            return Err(Error::from_errno(ret));
        }
    ...
    pub fn set_voltage(&mut self, min_uv: c_int, max_uv: c_int) -> Result {
        // SAFETY: The pointer is valid and non-null by the type invariant
        to_result(unsafe { bindings::regulator_set_voltage(self.0.as_ptr(), min_uv, max_uv) })
    }
    ...
    pub fn set_voltage_time(&mut self, old_uv: c_int, new_uv: c_int) -> Result<c_int> {
        // SAFETY: The pointer is valid and non-null by the type invariant
        let ret = unsafe { bindings::regulator_set_voltage_time(self.0.as_ptr(), old_uv, new_uv) };
        if ret < 0 {
            return Err(Error::from_errno(ret));
        }
    ...
    pub fn sync_voltage(&mut self) -> Result {
        // SAFETY: The pointer is valid and non-null by the type invariant
        to_result(unsafe { bindings::regulator_sync_voltage(self.0.as_ptr()) })
    }

    /// Get regulator output current
    pub fn get_current_limit(&self) -> Result<c_int> {
        // SAFETY: The pointer is valid and non-null by the type invariant
        let ret = unsafe { bindings::regulator_get_current_limit(self.0.as_ptr()) };
        if ret < 0 {
            return Err(Error::from_errno(ret));
        }
    ...
    pub fn set_current_limit(&mut self, min_ua: c_int, max_ua: c_int) -> Result<c_int> {
        // SAFETY: The pointer is valid and non-null by the type invariant
        let ret = unsafe { bindings::regulator_set_current_limit(self.0.as_ptr(), min_ua, max_ua) };
        if ret < 0 {
            return Err(Error::from_errno(ret));
        }
    ...
    pub fn set_load(&mut self, load_ua: c_int) -> Result {
        // SAFETY: The pointer is valid and non-null by the type invariant
        to_result(unsafe { bindings::regulator_set_load(self.0.as_ptr(), load_ua) })
    }

    /// Allow the regulator to go into bypass mode
    pub fn allow_bypass(&mut self, allow: bool) -> Result {
        // SAFETY: The pointer is valid and non-null by the type invariant
        to_result(unsafe { bindings::regulator_allow_bypass(self.0.as_ptr(), allow) })
    }

    /// Set the mode of the regulator
    pub fn set_mode(&mut self, mode: Mode) -> Result {
        // SAFETY: The pointer is valid and non-null by the type invariant
        to_result(unsafe { bindings::regulator_set_mode(self.0.as_ptr(), mode as _) })
    }

    /// Get the current mode of the regulator
    pub fn get_mode(&mut self) -> Result<Mode> {
        // SAFETY: The pointer is valid and non-null by the type invariant
        Mode::try_from(unsafe { bindings::regulator_get_mode(self.0.as_ptr()) })
    }
}
...
impl Drop for Regulator {
    fn drop(&mut self) {
        // SAFETY: The pointer is valid and non-null by the type invariant
        unsafe { bindings::regulator_put(self.0.as_ptr()) }
    }
}
...
impl PartialEq<Regulator> for Regulator {
    fn eq(&self, other: &Regulator) -> bool {
        ...
        unsafe { bindings::regulator_is_equal(self.0.as_ptr(), other.0.as_ptr()) }
    }
}
