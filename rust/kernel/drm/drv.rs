...
pub struct Registration<T: Driver> {
    ...
    vtable: Pin<KBox<bindings::drm_driver>>,
    _p: PhantomData<T>,
    _pin: PhantomPinned,
}
...
impl<T: Driver> Registration<T> {
    ...
    pub fn new<D: AsRef<device::Device>>(parent: D) -> Result<Self> {
        let vtable = Pin::new(KBox::new(Self::VTABLE, GFP_KERNEL)?);
        let raw_drm = unsafe { bindings::drm_dev_alloc(&*vtable, parent.as_ref().as_raw()) };
